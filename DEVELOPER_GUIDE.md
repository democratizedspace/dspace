...
app build SHA with `docs_meta.json`. Local dev builds fall back to `v3:dev-local` when
`VITE_GIT_SHA` is not set or git resolution fails, so prompt metadata is always explicit.
