...
These manifests deploy the `dspace-app` container built from the root `Dockerfile`. They align with
the k3s clusters we provision via sugarkube (see `docs/k3s-sugarkube-dev.md`) and reuse the same
port and health settings expected by the Raspberry Pi staging environment.
