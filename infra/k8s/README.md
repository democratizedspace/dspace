...
for the staging cluster. For the steady-state staging and production release flow, use the
[DSPACE Sugarkube release runbook](../../docs/ops/sugarkube-release.md).
...
The deployment uses pre-built images from GHCR by default. Sugarkube staging and production should
deploy published GHCR artifacts, not locally built Docker images:
...
Local Docker builds are for local development only and are not the normal Sugarkube staging or
production path. To build and load the image locally for k3s:
