...
DSPACE avoids traditional user accounts; authentication only occurs when you submit a custom quest to GitHub.
...
5. You can clear the saved token using the **Clear Token** button or by revoking it in your GitHub settings.

Tokens are never sent to any server other than GitHub. Remember to revoke the token when you no longer need it.
