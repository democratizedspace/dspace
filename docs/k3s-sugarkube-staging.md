...
- `env=staging` as the target environment for sugarkube commands (`env=int` is a deprecated alias)
...
- Staging should keep QA Cheats enabled. Set `DSPACE_ENV=staging` (via values files or chart env) so
  QA-only UI remains available for verification. Production must disable QA Cheats; dev keeps them
  enabled.
