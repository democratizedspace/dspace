...
> QA Cheats stay **enabled** in staging (`DSPACE_ENV=staging`). The production site
> (`democratized.space`) currently runs v2.x from the `main` branch and is managed separately.
...
- `kubectl` is pointed at the cluster and `just` is installed where you will run the sugarkube
  commands.
- Set `SUGARKUBE_TOKEN_STAGING` when joining an existing staging control plane.
