...
1. **Global navigation**
   ...
   - Verify focus indicators meet contrast requirements (WCAG 2.2.1) by visually checking against the
     background or using the design token contrast matrix.
2. **Quest selection**
...
3. **Quest run**
   ...
   - Capture an accessibility tree snapshot (Playwright or browser devtools) after the first decision
     point and attach it to the release notes archive.
4. **Settings panel**
...
5. **Regression capture**
   ...
   - Record color-contrast exceptions and temporary mitigations with owner + expiry date.
