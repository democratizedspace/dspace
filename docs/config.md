...
| `DSPACE_TOKEN_PLACE_URL`        | No (default `https://token.place`) | Deployment-time public token.place origin for SSR `/chat` props and `/config.json`. The server normalizes trailing slashes and legacy `/api` or `/api/v1` suffixes before Chat appends `/api/v1/chat/completions`. | `https://staging.token.place` | ConfigMap / values.yaml                                                   |
| `DSPACE_TOKEN_PLACE_CHAT_MODEL` | No (default `gpt-5-chat-latest`)   | Deployment-time public token.place API v1 chat model for SSR `/chat` props and `/config.json`.                                                                                                                     | `gpt-5-chat-latest`           | ConfigMap / values.yaml                                                   |
...
| `VITE_TOKEN_PLACE_URL`          | No (default `https://token.place`) | Local/build compatibility fallback used by the browser token.place client when SSR runtime props are unavailable. Do not create alternate Vite names.                                                              | `https://staging.token.place` | Local dev / build fallback                                                |
| `VITE_TOKEN_PLACE_CHAT_MODEL`   | No (default `gpt-5-chat-latest`)   | Local/build compatibility fallback for the token.place API v1 chat model when SSR runtime props are unavailable. Do not create alternate Vite names.                                                               | `gpt-5-chat-latest`           | Local dev / build fallback                                                |
...
> **token.place routing**: deployed SSR builds use `DSPACE_TOKEN_PLACE_URL` and
> `DSPACE_TOKEN_PLACE_CHAT_MODEL` as public, non-secret deployment configuration. The resolved
> values are exposed in `/config.json` and passed directly into hydrated `/chat` during SSR, so one
> immutable DSPACE image can be promoted between staging and production while Helm/runtime values
> choose the token.place origin. `VITE_TOKEN_PLACE_URL` and `VITE_TOKEN_PLACE_CHAT_MODEL` remain
> local/build compatibility fallbacks for dev, tests, and non-SSR fallback paths; they are no longer
> the authoritative deployment-routing mechanism.
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  `offlineWorker.enabled` boolean, a `telemetry.enabled` boolean (default `false`), and the
  resolved `tokenPlace.url` / `tokenPlace.model` deployment routing values. Setting
