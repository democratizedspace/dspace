...
| `DSPACE_DEFAULT_CHAT_PROVIDER`  | No (default `token-place`)         | Strict deployment default for Chat. Accepted values are exactly `token-place` and `openai`; invalid explicit values make runtime config unavailable. A valid saved user choice always takes precedence.                    | `openai`                      | Helm `chat.defaultProvider`                                               |
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  public `tokenPlace.url` / `tokenPlace.model` values and `chat.defaultProvider` showing the active Chat deployment
  routing. Setting
...
- `chat.defaultProvider` applies only when saved browser state is missing or has an invalid
  provider. It never overwrites a valid saved `token-place` or `openai` choice. OpenAI remains
  locally key-gated even when it is the deployment default.
