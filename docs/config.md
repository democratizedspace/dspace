...
| `VITE_TOKEN_PLACE_URL`        | No (default `https://token.place`) | Build-time origin embedded in the hydrated `/chat` client before appending `/api/v1/chat/completions`. Staging images/bundles must be built with `https://staging.token.place`; changing only runtime ConfigMaps after build does not update the browser bundle. | `https://staging.token.place` | Build environment / image build values                                    |
| `VITE_TOKEN_PLACE_CHAT_MODEL` | No (default `gpt-5-chat-latest`)   | Build-time model name embedded in the hydrated `/chat` client for token.place API v1 Chat completions. Use only for deployment-level compatibility or rollback mitigation.                                                                                       | `gpt-5-chat-latest`           | Build environment / image build values                                    |
...
> **Build-time Vite variables**: `VITE_TOKEN_PLACE_URL` and
> `VITE_TOKEN_PLACE_CHAT_MODEL` are read by the browser-hydrated Chat client through Vite's
> `import.meta.env` injection. For standard built deployments, provide them during `npm run build`
> or the image build. Runtime-only ConfigMap or Helm value changes will not alter the already
> compiled client bundle unless the deployment also exposes a separate runtime config path.
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  `offlineWorker.enabled` boolean, and a `telemetry.enabled` boolean (default `false`). Setting
