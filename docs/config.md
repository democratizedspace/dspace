...
| `DSPACE_TOKEN_PLACE_CHAT_MODEL` | No (default `gpt-5-chat-latest`)   | Deployment-time public token.place API v1 Chat model passed by SSR into hydrated `/chat` clients.                                                                                                                       | `gpt-5-chat-latest`           | Runtime environment / ConfigMap / values.yaml                             |
...
| `VITE_TOKEN_PLACE_CHAT_MODEL`   | No (default `gpt-5-chat-latest`)   | Local/build compatibility fallback used only when a runtime `DSPACE_TOKEN_PLACE_CHAT_MODEL` prop or explicit caller model is absent.                                                                                    | `gpt-5-chat-latest`           | Local dev / image build fallback                                          |
