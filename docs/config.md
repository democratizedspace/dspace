...
| `DSPACE_TOKEN_PLACE_URL`        | No (default `https://token.place`)   | Deployment-time public token.place origin passed by SSR into hydrated `/chat` clients before appending relay E2EE API v1 route paths such as `/api/v1/relay/servers/next`, `/api/v1/relay/requests`, and `/api/v1/relay/responses/retrieve`. Normalize origin, `/api`, and `/api/v1` values to avoid duplicate API path segments. | `https://staging.token.place` | Runtime environment / ConfigMap / values.yaml                             |
| `DSPACE_TOKEN_PLACE_CHAT_MODEL` | No (default `llama-3.1-8b-instruct`) | Deployment-time public token.place API v1 Chat model passed by SSR into hydrated `/chat` clients.                                                                                                                                                                                                                                 | `llama-3.1-8b-instruct`       | Runtime environment / ConfigMap / values.yaml                             |
...
| `VITE_TOKEN_PLACE_CHAT_MODEL`   | No (default `llama-3.1-8b-instruct`) | Local/build compatibility fallback used only when a runtime `DSPACE_TOKEN_PLACE_CHAT_MODEL` prop or explicit caller model is absent.                                                                                                                                                                                              | `llama-3.1-8b-instruct`       | Local dev / image build fallback                                          |
...
> **token.place deployment routing**: Deployed SSR builds use `DSPACE_TOKEN_PLACE_URL` and
> `DSPACE_TOKEN_PLACE_CHAT_MODEL` as public, non-secret runtime configuration. `/chat` resolves
> these on the server and passes them directly into the hydrated Chat panel, so one immutable
> DSPACE image can be promoted between staging and production while Helm/runtime values choose
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  public `tokenPlace.url` / `tokenPlace.model` values showing the active Chat deployment
  routing. Setting
