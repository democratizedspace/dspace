...
| `DSPACE_TOKEN_PLACE_CHAT_MODEL` | No (default `llama-3-8b-instruct`) | Deployment-time public token.place API v1 Chat model passed by SSR into hydrated `/chat` clients.                                                                                                                       | `llama-3-8b-instruct`         | Runtime environment / ConfigMap / values.yaml                             |
...
| `VITE_TOKEN_PLACE_CHAT_MODEL`   | No (default `llama-3-8b-instruct`) | Local/build compatibility fallback used only when a runtime `DSPACE_TOKEN_PLACE_CHAT_MODEL` prop or explicit caller model is absent.                                                                                    | `llama-3-8b-instruct`         | Local dev / image build fallback                                          |
