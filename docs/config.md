...
| `DSPACE_TOKEN_PLACE_URL`        | No (default `https://token.place`) | Public deployment runtime origin for direct browser token.place API v1 Chat calls. The SSR `/chat` route passes the resolved value into the hydrated client, and `/config.json` reports it for operator verification. Legacy `/api` and `/api/v1` suffixes are normalized away before appending `/api/v1/chat/completions`. | `https://staging.token.place` | ConfigMap / values.yaml                                                   |
| `DSPACE_TOKEN_PLACE_CHAT_MODEL` | No (default `gpt-5-chat-latest`)   | Public deployment runtime model for token.place API v1 Chat completions. Use runtime values to route the same immutable DSPACE image differently in staging and production.                                                                                                                                                 | `gpt-5-chat-latest`           | ConfigMap / values.yaml                                                   |
| `VITE_TOKEN_PLACE_URL`          | No (default `https://token.place`) | Local/build compatibility fallback used only when no explicit caller, saved compatibility URL, or SSR runtime value is supplied to the token.place client. Deployed SSR builds should prefer `DSPACE_TOKEN_PLACE_URL`.                                                                                                      | `https://staging.token.place` | Local development / build fallback                                        |
| `VITE_TOKEN_PLACE_CHAT_MODEL`   | No (default `gpt-5-chat-latest`)   | Local/build compatibility fallback used only when no explicit caller or SSR runtime model is supplied. Deployed SSR builds should prefer `DSPACE_TOKEN_PLACE_CHAT_MODEL`.                                                                                                                                                   | `gpt-5-chat-latest`           | Local development / build fallback                                        |
...
> **token.place deployment routing**: deployed SSR builds should set
> `DSPACE_TOKEN_PLACE_URL` and `DSPACE_TOKEN_PLACE_CHAT_MODEL`. The server resolves those public,
> non-secret values at request time, exposes them through `/config.json`, and passes them as
> hydrated `/chat` props. The existing `VITE_TOKEN_PLACE_URL` and
> `VITE_TOKEN_PLACE_CHAT_MODEL` names remain compatibility fallbacks for local builds and direct
> client helper usage; they are no longer the authoritative way to distinguish staging from
> production. One immutable DSPACE image can be promoted between environments while Helm/runtime
> values choose whether Chat calls `https://staging.token.place` or `https://token.place`.
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  `offlineWorker.enabled` boolean, a `telemetry.enabled` boolean (default `false`), and
  `tokenPlace.url` / `tokenPlace.model` public deployment routing values. Setting
