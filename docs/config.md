...
| `DSPACE_DEFAULT_CHAT_PROVIDER` | No (default `token-place`)          | Strict Chat deployment default: exactly `token-place` or `openai`. Missing/invalid saved state uses it; a valid user choice wins. OpenAI remains locally key-gated. | `openai` | Helm `chat.defaultProvider` |
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  `offlineWorker.enabled` boolean, a `telemetry.enabled` boolean (default `false`), a `chat.defaultProvider` deployment coordinate, and
