...
| `DSPACE_TOKEN_PLACE_URL`        | No (default `https://token.place`) | Public deployment-time token.place origin resolved by SSR for `/config.json` and the hydrated `/chat` panel before appending `/api/v1/chat/completions`. Staging sets `https://staging.token.place`; production uses `https://token.place`. | `https://staging.token.place` | ConfigMap / values.yaml                                                   |
| `DSPACE_TOKEN_PLACE_CHAT_MODEL` | No (default `gpt-5-chat-latest`)   | Public deployment-time token.place API v1 chat model resolved by SSR for `/config.json` and `/chat`.                                                                                                                                        | `gpt-5-chat-latest`           | ConfigMap / values.yaml                                                   |
| `VITE_TOKEN_PLACE_URL`          | No (default `https://token.place`) | Local/build compatibility fallback for token.place origin when SSR runtime props are unavailable. Deployed SSR images should prefer `DSPACE_TOKEN_PLACE_URL` so one immutable image can move between environments.                          | `https://staging.token.place` | Local dev / compatibility build environment                               |
| `VITE_TOKEN_PLACE_CHAT_MODEL`   | No (default `gpt-5-chat-latest`)   | Local/build compatibility fallback for the token.place model when SSR runtime props are unavailable.                                                                                                                                        | `gpt-5-chat-latest`           | Local dev / compatibility build environment                               |
...
> **token.place runtime routing**: Deployed SSR builds should set `DSPACE_TOKEN_PLACE_URL`
> and `DSPACE_TOKEN_PLACE_CHAT_MODEL` at deployment time. Astro resolves these server-side,
> exposes the safe public values in `/config.json`, and passes them directly into the hydrated
> Chat panel. The existing `VITE_TOKEN_PLACE_URL` and `VITE_TOKEN_PLACE_CHAT_MODEL` names remain
> local/build compatibility fallbacks; they are not new deployment knobs and no alternate Vite
> names should be introduced. This allows one immutable DSPACE image to be promoted from staging
> to production while Helm/runtime values choose the active token.place origin and model.
...
- `GET /config.json` streams runtime toggles so operators can verify feature flags without
  ...
  `tokenPlace.url` / `tokenPlace.model` public deployment configuration. Setting
