...
## v3.0.0 launch parity notes

- **Staging vs prod toggle checks**: compare only critical runtime toggles (`DSPACE_FEATURE_FLAGS`,
  telemetry override, offline worker setting). Hostnames, secrets, and endpoint values are expected
  to differ per environment.
- **Build provenance**: for release validation and rollout, pin immutable image tags (`sha-<commit>`
  or `<branch>-<shortsha>`) generated by CI; do not promote local-only images.
- **Cache/CDN review focus**: after deploy, verify `/config.json` reflects the current rollout and
  confirm service-worker updates do not keep stale runtime flags cached between environments.
