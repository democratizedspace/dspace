...
3. **Verify locally (no tunnel by default):**
   ...
   - In another shell, run:
     - `curl -fsS http://localhost:3000/healthz` (readiness)
     - `curl -fsS http://localhost:3000/livez` (liveness)
   - Then open http://localhost:3000/ in a browser for a UI smoke check.
...
- Cloudflare tunnels are optional in dev. If you add one, align it with `env=dev` and a non-public
  hostname.
- `v3-latest` is fine for day-to-day dev iteration.
- Prefer immutable tags (`v3-<shortsha>` or semver tags) when reproducing bugs, comparing behavior
  across nodes, or documenting a specific test run.
