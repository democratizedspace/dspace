...
- `electronics/potentiometer-dimmer`
  - [x] Observed issue (addressed): `electronics/potentiometer-dimmer` previously trended toward thin-shell progression with limited decision points and weak intermediate proof gates; this was hardened in this pass. (PR #0)
...
- `electronics/resistor-color-check`
  - [x] Observed issue (addressed): `electronics/resistor-color-check` previously centered on a measurement/check action but acceptance thresholds and out-of-range handling were thin; this was hardened in this pass. (PR #0)
...
- `electronics/servo-sweep`
  - [x] Observed issue (addressed): `electronics/servo-sweep` previously trended toward thin-shell progression with limited decision points and weak intermediate proof gates; this was hardened in this pass. (PR #0)
  - [x] Add at least one non-linear branch (main path plus alternate strategy) and gate advancement on mechanics-backed evidence (`requiresItems`, `launchesProcess`, or logged output). (PR #0)
  - [x] Insert troubleshooting/recovery handling so failures produce actionable next steps and a verification retry before `finish`; include domain-appropriate safety stop conditions where risk handling is relevant. (PR #0)
