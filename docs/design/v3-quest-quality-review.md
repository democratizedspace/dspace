...
- `hydroponics/ec-check`
  - [x] Observed issue: `hydroponics/ec-check` centers on a measurement/check action but acceptance thresholds and out-of-range handling are thin.
...
- `hydroponics/filter-clean`
  - [x] Observed issue: `hydroponics/filter-clean` describes a cleaning cycle without a measurable before/after success definition.
...
- `hydroponics/grow-light`
  ...
  - [ ] Insert troubleshooting/recovery handling so failures produce actionable next steps and a verification retry before `finish`.
- `hydroponics/lettuce`
  - [x] Observed issue: `hydroponics/lettuce` is lifecycle-oriented but can read as narration unless staged setup and outcome proofs are separated.
...
- `hydroponics/temp-check`
  ...
  - [ ] Add an out-of-range corrective branch with a mandatory re-test loop and explicit pass/fail bounds.
- `hydroponics/top-off`
  - [x] Observed issue: `hydroponics/top-off` still trends toward thin-shell progression with limited decision points and weak intermediate proof gates.
