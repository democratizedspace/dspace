...
- The `/chat` page hydrates one provider-aware Svelte chat panel via
  `frontend/src/pages/chat/index.astro`, `frontend/src/pages/chat/svelte/Integrations.svelte`,
  and `frontend/src/pages/chat/svelte/ChatPanel.svelte`.
- token.place is the default provider and calls `runTokenPlaceChatCompletion` from
  `frontend/src/utils/tokenPlace.js`.
- OpenAI remains an opt-in provider selected from `/settings` and calls `GPT5ChatV2` from
  `frontend/src/utils/openAI.js` only when a user-saved OpenAI key is present.
...
- Persona system prompts are defined in `frontend/src/data/npcPersonas.js` and included in the
  shared provider-aware chat prompt.
- `GPT5ChatV2` and `runTokenPlaceChatCompletion` return rich chat objects with `text` and
  DSPACE-owned `contextSources`; token.place also preserves optional provider `usage` and echoed
  safe `metadata` when API v1 returns them.
- token.place requests use API v1 `/api/v1/chat/completions` with `model`, `messages`, and safe
  optional `metadata`, without streaming or token.place credentials.

### Error handling + uncertainty

- OpenAI errors are mapped to user-facing copy in `frontend/src/utils/openAI.js`.
- token.place errors are mapped by `frontend/src/utils/tokenPlaceErrors.js` so UI banners avoid
  leaking raw provider internals.
- The provider-aware Chat panel can show context sources, prompt debug details, and token.place
...
- **Knowledge coverage:** add the custom content docs + routes to the knowledge summary so the
  ...
  `docs/ROUTES.md`.
- **Deterministic retrieval:** force-include custom content doc chunks for editor/import/export/
  backup/schema intents to prevent lexically missed coverage in the RAG excerpt packer.
