...
Status: v3.1.0 is not treated as shipped by this document. The repository release metadata is
prepared for `3.1.0`, and the active GHCR Helm publishing workflow packages `charts/dspace`,
whose chart and app versions are also prepared as `3.1.0`. Forward-looking roadmap or QA text must
therefore be read as release planning until a v3.1.0 tag, immutable image, published chart, staging
...
| Canonical GHCR chart          | `.github/workflows/ci-helm.yml` packages and publishes `charts/dspace` to `oci://ghcr.io/democratizedspace/charts/dspace`.                                                                              | Release-candidate metadata prepared for v3.1.0.       | `charts/dspace` is the canonical chart path for the current GHCR/Sugarkube release path.                                                                        |
