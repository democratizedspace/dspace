...
Status: v3.1.0 is not treated as shipped by this document. The application and application release
metadata are prepared for `3.1.0`, and the active GHCR Helm publishing workflow packages
`charts/dspace`, whose chart is prepared as `3.1.1` with `appVersion: 3.1.0`. Forward-looking
roadmap or QA text must therefore be read as release planning until a v3.1.0 tag, immutable image,
published chart, staging evidence, and production promotion are recorded.
...
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime version               | `package.json` is prepared as `3.1.0`.                                                                                                                                                                                                        | Release-candidate metadata prepared.                                   | Do not claim v3.1.0 shipped until immutable artifacts, staging evidence, and production promotion exist.                                                                    |
...
| Canonical GHCR chart          | `.github/workflows/ci-helm.yml` packages and publishes `charts/dspace` to `oci://ghcr.io/democratizedspace/charts/dspace`.                                                                                                                    | Chart 3.1.1 for app 3.1.0 prepared.                                    | `charts/dspace` is the canonical chart path for the current GHCR/Sugarkube release path.                                                                                    |
