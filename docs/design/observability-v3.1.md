...
Status: v3.1.0 is not treated as shipped by this document. The repository metadata is prepared
for release candidate `3.1.0`, and the active GHCR Helm publishing workflow packages
`charts/dspace`, whose chart version and appVersion are also prepared as `3.1.0`.
Forward-looking roadmap or QA text must therefore be read as release planning until a v3.1.0 tag,
immutable image, chart publication, staging evidence, and production promotion are recorded.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime version               | `package.json` is prepared as `3.1.0`.                                                                                                                                                                   | Release-candidate metadata prepared; artifacts not published. | Do not claim v3.1.0 shipped until immutable artifacts and promotion evidence exist.                                                                             |
...
| Canonical GHCR chart          | `.github/workflows/ci-helm.yml` packages and publishes `charts/dspace` to `oci://ghcr.io/democratizedspace/charts/dspace`.                                                                              | Release-candidate chart metadata prepared for 3.1.0. | `charts/dspace` is the canonical chart path for the current GHCR/Sugarkube release path; publication is still pending.                                          |
