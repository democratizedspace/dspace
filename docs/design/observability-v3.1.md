...
for release-candidate validation with `package.json` version `3.1.0`, and the active GHCR Helm
publishing workflow packages `charts/dspace`, whose chart version and appVersion are also
`3.1.0`. Forward-looking roadmap or QA text must still be read as release planning until a
v3.1.0 tag, immutable image, chart artifact, staging evidence, and production promotion are
recorded.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime version               | `package.json` is `3.1.0`.                                                                                                                                                                              | Release-candidate metadata prepared; artifacts pending. | Do not claim v3.1.0 shipped until immutable image/chart artifacts and release evidence exist.                                                                   |
...
| Canonical GHCR chart          | `.github/workflows/ci-helm.yml` packages and publishes `charts/dspace` to `oci://ghcr.io/democratizedspace/charts/dspace`; the source chart reports version/appVersion `3.1.0`.                         | Release-candidate metadata prepared; artifacts pending. | `charts/dspace` is the canonical chart path for the current GHCR/Sugarkube release path.                                                                        |
