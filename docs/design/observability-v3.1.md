...
metadata are prepared for `3.1.1`, and the active GHCR Helm publishing workflow packages
`charts/dspace`, whose chart is prepared as `3.1.2` with `appVersion: 3.1.1`. Forward-looking
...
| Canonical GHCR chart          | `.github/workflows/ci-helm.yml` packages and publishes `charts/dspace` to `oci://ghcr.io/democratizedspace/charts/dspace`.                                                                                                                    | Chart 3.1.2 for app 3.1.1 prepared.                                    | `charts/dspace` is the canonical chart path for the current GHCR/Sugarkube release path.                                                                                    |
