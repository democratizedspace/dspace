...
Status: v3.1.0 is not treated as shipped by this document. Repository release metadata is
prepared for DSPACE 3.1.0, and the active GHCR Helm publishing workflow packages `charts/dspace`
with chart version `3.1.0`, appVersion `3.1.0`, and default image tag `v3.1.0`. Forward-looking
roadmap or QA text must still be read as release planning until a v3.1.0 tag, immutable image,
chart artifact, staging evidence, and production promotion are recorded.
...
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime version               | `package.json` is prepared as `3.1.0`.                                                                                                                                                                                                        | Release-candidate metadata prepared; live v3.1.0 evidence pending.     | Do not claim v3.1.0 shipped until release metadata and immutable artifacts exist.                                                                                           |
...
| Canonical GHCR chart          | `.github/workflows/ci-helm.yml` packages and publishes `charts/dspace` to `oci://ghcr.io/democratizedspace/charts/dspace`; release-candidate chart metadata is `3.1.0`.                                                                       | Canonical v3.1.0 metadata prepared; artifact publication pending.      | `charts/dspace` is the canonical chart path for the current GHCR/Sugarkube release path.                                                                                    |
