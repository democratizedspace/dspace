...
### 3.1) `/quests` TTI performance validation (launch-gating)

> Primary validation for this patch: **DevTools trace + User Timing + Playwright harness**.
> Lighthouse is supplementary only.

- [ ] Run `/quests` Playwright timing harness against staging (or release-candidate env)
- [ ] Record candidate timings for:
  - [ ] `quests:builtin-visible`
  - [ ] `quests:snapshot-classification-ready`
  - [ ] `quests:full-state-reconciled`
  - [ ] `quests:custom-merged`
- [ ] Confirm User Timing entries exist and values are sensible/non-negative
- [ ] Capture at least one Chrome/Chromium DevTools Performance trace for `/quests`
- [ ] Verify the trace shows improved critical path for first meaningful list content
- [ ] (Optional) Run Lighthouse locally as supporting context; do not use it as sole sign-off
- [ ] Paste timing/trace references in release notes before promotion

Suggested commands:

```bash
# run app preview for local dry run
npm run build && npm run preview
```

```bash
# local or staging target, optionally simulate slower CPU
QUESTS_PERF_BASE_URL=https://staging.democratized.space \
QUESTS_PERF_CPU_SLOWDOWN=4 \
