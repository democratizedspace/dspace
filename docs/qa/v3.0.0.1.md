...
### `/quests` TTI optimization launch gate (required for this feature)

Primary validation stack for this feature (must be completed before launch):

1. Chrome/Chromium DevTools Performance trace review
2. App-specific User Timing marks/measures
3. Playwright measurement harness repeat runs

Lighthouse is supplementary only and cannot replace the three checks above.

- [ ] Run `/quests` perf harness on staging/release candidate and capture output:

```bash
npm --prefix frontend run perf:quests
```

- [ ] (Optional low-end simulation) rerun with CPU throttling and capture output:

```bash
QUESTS_PERF_CPU_SLOWDOWN=4 npm --prefix frontend run perf:quests
```

- [ ] Record candidate timing numbers (attach in release notes):
  - [ ] `quests:time-to-builtins-visible`: `__________ ms`
  - [ ] `quests:time-to-snapshot-classification`: `__________ ms`
  - [ ] `quests:time-to-fullstate-reconciliation`: `__________ ms`
  - [ ] `quests:time-to-custom-merge` (if present): `__________ ms`
- [ ] Confirm User Timing milestones exist and are sensible:
...
- [ ] Capture and review at least one Chrome DevTools Performance trace on `/quests` to confirm
      critical-path improvement and no late blocking work before meaningful list content appears.
- [ ] Optional supporting audit only (not a gate by itself):

```bash
npx lighthouse https://staging.democratized.space/quests --view
```
