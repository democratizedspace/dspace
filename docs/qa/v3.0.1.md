...
- [ ] QA signoff confirms the audited commit delta above resolves to the immutable rc.6 SHA and
      ...
      checklist item)
  - Evidence pending (2026-06-01 UTC, Codex checkout): no configured `origin` remote is available
    in this checkout, so the SHA/range resolution gate remains open until maintainers attach remote
    tag evidence. The release closeout must attach remote tag proof from
    `git ls-remote --tags origin v3.0.1-rc.6`, plus local `git rev-parse v3.0.1-rc.6`,
    `git merge-base --is-ancestor 3ec45a5517a35c96767f6b946c01104e6ec88f93 v3.0.1-rc.6`,
    `git log --oneline`, `git diff --name-only`, and `git diff --stat` evidence for
    `3ec45a5517a35c96767f6b946c01104e6ec88f93..v3.0.1-rc.6`, confirming the expected candidate
    SHA `8a1fa6ca2b4206c3e481da6b8f02c912b56dfdb0` and audited range before this gate is closed.
...
- [x] Roll back immediately to previous known-good immutable tag
- [x] Re-run prod health checks
- [x] Log incident notes and keep `v3.0.1` open until replacement candidate is validated
...
- [x] Final release git tag created and pushed (`v3.0.1`)
- [x] `v3.0.1` patch-note addendum published in changelog
- [x] Deployed immutable tag + SHA recorded in release notes
- [x] Evidence links attached (perf outputs, traces, smoke logs, key manual QA notes)
- [x] Confirmed that deferred/future work is tracked in `v3.1+`, not carried silently in patch scope
