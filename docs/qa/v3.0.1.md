...
- [x] QA signoff confirms this checklist and executed test scope were derived from the audited commit delta above
  ...
  Evidence note (2026-05-17 UTC, Codex automation): refreshed the required commit-range audit with
  `git log --oneline`, `git diff --name-only`, and `git diff --stat` from the v3 baseline through
  both the rc.4 candidate (`2d7f93daa4869ae223825cbd00a37bc83ac5703e`) and current `main`. The
  v3.0.1 checklist scope still matches the audited rc.4 patch delta for `/quests`, `/processes`,
  `/docs`, `/chat`, and release-safety gates, while
  `frontend/src/pages/docs/md/changelog/20260401.md` now carries the broader v3.1 user-visible
  changelog coverage for the full audited main-branch delta.
...
- [x] QA owner + timestamp: `Codex automation (GPT-5.3-Codex), 2026-04-17 UTC`
