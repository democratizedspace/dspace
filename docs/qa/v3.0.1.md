...
- `frontend/e2e/release-v3-0-1-launch-critical.spec.ts`
  - `keeps built-in and custom quests coexisting during delayed merge and navigation`
  - `renders summary-first process rows and preserves built-in/custom coexistence routing`
  - `keeps detail controls active and buy-required behavior working on built-in process detail`
