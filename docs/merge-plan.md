...
- Ensure CI is green for the release pipeline (image build and Helm chart publish jobs succeeded).
...
- Verify the rollback plan:
  ...
  - Document how to switch images if needed:
    ...
    ```
- Confirm chart appVersion and the deployed image tag match the v3 release being merged:
