...
- [ ] Document the Helm values diff between dev and main:
  - [ ] Export both value sets and compare.
  - [ ] Note any deviations that will ship to main.
  - Shell snippet:
    ```bash
    helm get values dspace -n <NAMESPACE> --all > /tmp/values-main.yaml
...
- [ ] Verify rollback plan:
  ...
  - [ ] Document how to switch images if needed:
    ...
    ```
- [ ] Confirm chart appVersion and deployed image tag match main:
  ...
  - [ ] Check the image tag in values for main:
    ...
    ```
  - [ ] Ensure both values align with the image built for main.
