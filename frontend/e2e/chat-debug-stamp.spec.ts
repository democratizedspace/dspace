...
test.describe('Chat debug build stamp', () => {
    ...
    test('shows non-missing prompt/app build stamps', async ({ page }) => {
        ...
        const promptVersion = debugPanel.locator('.debug-version');
        await expect(promptVersion).toContainText('Prompt version:');
        await expect(promptVersion).not.toContainText('v3:missing');
        await expect(promptVersion).not.toContainText('v3:dev-local');

        const appShaRow = debugPanel.getByTestId('debug-app-sha-row');
        const appShaValue = appShaRow.locator('.debug-mono');
        await expect(appShaValue).toBeVisible();
        await expect(appShaValue).not.toHaveText('');
        await expect(appShaValue).not.toContainText('missing');
        ...
        await expect(appSourceValue).not.toHaveText('');
        ...
        await expect(comparisonValue).not.toContainText(/app sha missing/i);
    });
});
