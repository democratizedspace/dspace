...
test.describe('Game Save System Functionality', () => {
    ...
    test('should load gamesaves page', async ({ page }) => {
        ...
        const pageTitle = page.locator('h1, h2').filter({ hasText: /gamesaves|saves|save games/i }).first();
    ...
    test('should display save game button', async ({ page }) => {
        ...
        const saveButton = page.locator('button, a').filter({ hasText: /save game|save$/i }).first();
        
        if (await saveButton.count() === 0) {
            // Try looking in menu or settings
            const menuButton = page.locator('button, a').filter({ hasText: /menu|settings|options/i }).first();
            
            if (await menuButton.count() > 0) {
                await menuButton.click();
                await page.waitForTimeout(500);
                
                const saveButtonInMenu = page.locator('button, a').filter({ hasText: /save game|save$/i }).first();
    ...
    test('should create new save', async ({ page }) => {
        ...
        const confirmationMsg = page.locator('div, p').filter({ hasText: /saved|game saved|save created/i }).first();
    ...
    test('should load saved game', async ({ page }) => {
        ...
        if (!canSave) {
            ...
            await page.evaluate(() => {
                localStorage.setItem('dspace_test_save', JSON.stringify({
                    timestamp: Date.now(),
                    name: 'Test Save',
                    data: { test: true }
                }));
        ...
        const pageTitle = await page.locator('h1, h2').filter({ hasText: /gamesaves|saves|save games/i }).count();
        ...
        const loadButton = page.locator('button, a').filter({ hasText: /load game|load$/i }).first();
        ...
        if (await loadButton.count() === 0) {
            ...
            if (await saveEntry.count() > 0) {
                ...
                const hasSaveData = await page.evaluate(() => {
                    return Object.keys(localStorage).some(key => 
                        key.includes('current') || key.includes('active') || key.includes('game'));
                });
            ...
            await page.waitForTimeout(500);
            const confirmationMsg = page.locator('div, p').filter({ hasText: /loaded|game loaded/i }).first();
    ...
    test('should auto-save game state', async ({ page }) => {
        ...
        const hasAutoSave = await page.evaluate(() => {
            return Object.keys(localStorage).some(key => 
                key.includes('auto') || key.includes('save') || key.includes('game') || key.includes('progress'));
        });
        ...
        if (hasAutoSave) {
            ...
            if (await menuButton.count() > 0) {
                ...
                const autoSaveOption = page.locator('div, label, span').filter({ 
                    hasText: /auto-?save/i 
                }).first();
