...
export async function waitForHydration(page: Page, target?: string): Promise<void> {
    ...
    if (target) {
        ...
        const resolvedSelector = isSelector ? target : `[data-hydrated="${target}"]`;
        const targetLocator = page.locator(resolvedSelector);
        await expect(targetLocator.first()).toBeVisible({ timeout: 10_000 });
        return;
    }

    const hydrationMarker = page.locator('[data-hydrated]');
    if (await hydrationMarker.count()) {
        await expect(hydrationMarker.first()).toBeVisible({ timeout: 10_000 });
    }
}
