...
const CRITERIA = {
  ...
  ETHICAL_KEYWORDS: ['appropriate', 'ethical', 'humane', 'welfare', 'proper care', 'adequate space'],
  NPC_PERSONALITIES: {
...
function checkDialogueStyle(quest) {
  ...
  quest.dialogue.forEach(node => {
    ...
    if (!matchesPersonality && text.length > CRITERIA.MIN_DIALOGUE_LENGTH) {
      issues.push(`Dialogue node ${node.id} in quest ${quest.id} may not match ${npcImage}'s personality`);
    }
...
function checkAquariumEthics(quest) {
  ...
  let questText = quest.dialogue.map(node => node.text).join(' ').toLowerCase();
  
  // Check for tank size mentions
  if (quest.id.includes('goldfish')) {
    if (!questText.includes('150') && !questText.includes('200')) {
      issues.push(`Goldfish quest ${quest.id} may not specify adequate tank size (150-200L minimum)`);
    }
    
    // Check for single goldfish recommendation
    if (questText.includes('two goldfish') && !questText.includes('150 liter') && !questText.includes('single goldfish')) {
      issues.push(`Goldfish quest ${quest.id} should recommend one goldfish for a tank under 200L`);
    }
  }
  ...
  if (quest.id.includes('goldfish')) {
    ...
    if (!questText.includes(minTemp.toString()) || !questText.includes(maxTemp.toString())) {
      issues.push(`Goldfish quest ${quest.id} should specify proper temperature range (${minTemp}-${maxTemp}°C)`);
    }
  }
  ...
  if (quest.id.includes('guppy')) {
    ...
    if (!questText.includes(minTemp.toString()) || !questText.includes(maxTemp.toString())) {
      issues.push(`Guppy quest ${quest.id} should specify proper temperature range (${minTemp}-${maxTemp}°C)`);
    }
  }
    ...
    if (category === 'aquaria') {
      ...
      const aquariaQuests = questIds
        .map(id => {
          ...
          const orderIndex = expectedOrder.findIndex(keyword => basename.includes(keyword));
      ...
      for (let i = 1; i < aquariaQuests.length; i++) {
        ...
        if (!hasPreviousLevelDependency && previousLevelQuests.length > 0) {
          issues.push(`Quest ${aquariaQuests[i].id} should depend on at least one of: ${previousLevelQuests.join(', ')}`);
        }
      }
    }
  }
