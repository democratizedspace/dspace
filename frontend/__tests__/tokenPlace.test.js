...
describe('token.place API v1 client', () => {
    ...
    test('runtime staging URL is used after explicit and state compatibility URLs', async () => {
        await tokenPlaceChat([{ role: 'user', content: 'hello' }], {
    ...
    test('explicit URL remains highest precedence over state, runtime, and Vite URLs', async () => {
    ...
    test('state tokenPlace url compatibility override works over runtime URL', async () => {
        process.env.VITE_TOKEN_PLACE_URL = 'https://env.token.place';
        loadGameState.mockReturnValue({ tokenPlace: { url: 'https://state.token.place/' } });
        await tokenPlaceChat([{ role: 'user', content: 'hello' }], {
            runtimeUrl: 'https://staging.token.place',
