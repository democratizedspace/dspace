...
// Polyfill TextDecoder before loading image-size
...
function findSimilarImages(missingImage) {
    ...
    const similarFiles = glob.sync(searchPattern).map((file) => {
        // Convert back to web path format
        const relativePath = path.relative(path.join(__dirname, publicDirectoryRelativePath), file);
    ...
    test('Images have correct dimensions', () => {
        if (incorrectDimensionImages.length > 0) {
            ...
            incorrectDimensionImages.forEach((image) => {
                console.warn(
                    `- ${image.path} (${image.actual.width}x${image.actual.height}, expected ${image.expected.width}x${image.expected.height})`
                );
            });
        }
