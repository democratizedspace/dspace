...
describe('gameState - common utilities', () => {
    ...
    test('sequential saves without awaiting merge changes', async () => {
        const first = loadGameState();
        first.inventory['1'] = 1;
        const saveA = saveGameState(first);

        const second = loadGameState();
        second.quests['q'] = true;
        const saveB = saveGameState(second);

        await saveA;
        await saveB;

        const final = loadGameState();
        expect(final.inventory['1']).toBe(1);
        expect(final.quests['q']).toBe(true);
    });
});
