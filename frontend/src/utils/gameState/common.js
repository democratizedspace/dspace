...
export const saveGameState = async (newState) => {
    ...
    state.set(gameState);
    const backupPromise = write(BACKUP_STORE, snapshot).catch(() => undefined);
    const statePromise = write(STATE_STORE, gameState).catch(() => undefined);
    await Promise.all([backupPromise, statePromise]);
};
