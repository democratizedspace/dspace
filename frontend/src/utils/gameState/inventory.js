...
export const getContainedItemCounts = (containerItemId, itemIds = []) => {
    const allStoredCounts = getStoredItemCounts(containerItemId);

    return itemIds.reduce((acc, itemId) => {
        acc[itemId] = Number(allStoredCounts[itemId] || 0);
        return acc;
    }, {});
};
...
export const addContainedItems = (containerItemId, itemId, count) => {
    ...
    addStoredItems(containerItemId, itemId, normalizedCount);
};
