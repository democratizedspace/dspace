...
const sharedSystemGuardrail = [
    ...
    'Only give exact counts/durations/rates if they appear in retrieved context; otherwise ' +
        "be approximate or say you don't know.",
].join(' ');
...
const applySystemGuardrail = (prompt) => {
    ...
    const hasGuardrailMarkers = guardrailMarkers.every((marker) =>
        normalizedPrompt.includes(marker)
    );
    if (hasGuardrailMarkers) return prompt;
