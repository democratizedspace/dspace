...
const buildLegacyV1SeedItems = (profileId: string): LegacyV1SeedItem[] => {
    ...
    cookies.forEach(({ name }) => {
        ...
        if (name.startsWith('currency-balance-')) {
            const symbol = name.replace('currency-balance-', '').trim();
            if (symbol) {
    ...
    const itemEntries = Array.from(itemIds)
        .sort((a, b) => a - b)
        .map((id) => {
    ...
    const currencyEntries = Array.from(currencySymbols)
        ...
        .map((symbol) => {
            const mappedId = V1_CURRENCY_SYMBOL_TO_V3_ITEM_ID[symbol] ?? 'UNMAPPED';
            const mapping =
                LEGACY_V1_ITEM_MAPPINGS.find((entry) => entry.v3Id === mappedId) ??
                LEGACY_V1_ITEM_MAPPINGS.find((entry) => entry.v1Name === symbol);

            return {
                v1Key: `currency-balance-${symbol}`,
                v1Name: mapping?.v1Name ?? symbol,
                v3Id: mapping?.v3Id ?? mappedId,
                v3Name: mapping?.v3Name ?? (mappedId === 'UNMAPPED' ? 'UNMAPPED' : symbol),
            };
        });

    return [...itemEntries, ...currencyEntries];
};
...
export const clearV3GameStateStorage = async (): Promise<boolean> => {
    ...
    let hasLegacyV2Data = false;
    LEGACY_V2_STORAGE_KEYS.forEach((key) => {
        const raw = localStorage.getItem(key);
        if (raw === null) return;
        const parsed = parseLegacyV2Raw(raw);
        if (parsed.isLegacy) {
            hasLegacyV2Data = true;
            return;
        }
        localStorage.removeItem(key);
        removedV3Keys = true;
