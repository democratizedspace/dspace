...
const TOKEN_PLACE_MODEL_ALIASES = {
    [LEGACY_LLAMA_CHAT_MODEL]: new Set([DEFAULT_CHAT_MODEL]),
};
...
const normalizeRelayModel = (model) =>
    typeof model === 'string' && model.trim() ? model.trim() : undefined;

const canResolveTokenPlaceModel = (requestedModel, resolvedModel) => {
    if (!requestedModel || !resolvedModel) return false;
    if (requestedModel === resolvedModel) return true;
    return TOKEN_PLACE_MODEL_ALIASES[requestedModel]?.has(resolvedModel) === true;
};

const getEffectiveRelayModel = (data, requestedModel) => {
    const echoedRequestedModel = normalizeRelayModel(getRelayRequestedModel(data));
    if (echoedRequestedModel && echoedRequestedModel !== requestedModel) {
        throw createMalformedTokenPlaceResponseError(
            'token.place relay selected contradictory requested model metadata.'
        );
    }

    const resolvedModel = normalizeRelayModel(getRelayResolvedModel(data));
    if (!resolvedModel) return requestedModel;
    if (!canResolveTokenPlaceModel(requestedModel, resolvedModel)) {
        throw createMalformedTokenPlaceResponseError(
