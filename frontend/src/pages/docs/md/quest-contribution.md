...
2. **Build your quest** following the [Quest Schema](/docs/quest-schema). Include at least one item
   or process reference.
3. **Run validations**
    ```bash
    node scripts/validate-quest.js path/to/quest.json
    ```
    This checks schema compliance, hardening metadata, and quest/process references. Maintainers
    also run `npm run hardening:validate` against the full catalog.
4. **Bundle related content**
    ```bash
    node scripts/create-content-bundle.js
    ```
    This creates a JSON bundle in `submissions/bundles`.
5. **Submit via the in-game form** at `/quests/submit` (single quest JSON) or `/bundles/submit`
   (bundle JSON). See the [Quest Submission Guide](/docs/quest-submission) for details on exporting
   JSON from the in-game editor.
6. **Authorize GitHub** with a personal access token in the submission form and create a pull
   request.
