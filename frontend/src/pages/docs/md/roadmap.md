---
title: 'Roadmap'
slug: 'roadmap'
---

This is just a tentative roadmap showing what I'm hoping to work on next. Things are subject to
change, timelines are subject to delays, and some things may be reprioritized.

## 2027 and beyond

Multiplayer remains a long-term research area; updates will follow once prototypes reach
community testing.
- Guilds launch once the v3-era social systems are mature (targeting 2027).

## 2026

- Custom quests ship with DSPACE v3 (targeting March 2026).
- Smarter dChat ships with DSPACE v3 (targeting March 2026).
- 10x more quests arrives alongside DSPACE v3 (targeting March 2026).
- Top-down isometric base building remains a late-2026 target once v3 systems are stable.

## 2025

The bulk of the year is dedicated to building DSPACE v3 and its supporting systems.

## 2023

- [x] in-game guided model rocket hop (servo + camera upgrade)
- [x] AI-enabled NPCs (chat now supports persona-powered conversations)

## July 2023

- [x] content update (suborbital launch checklist for rockets)

## June 2023

- [x] [DSPACE v2 release notes](/changelog#20230630)

## January 2023

- [x] goldfish aquarium
- [x] rocket with a parachute
- [x] dSolar
- [x] basic income
- [x] EV charger
- [x] carbon offsets
- [x] 1kwh battery

## October 2022

- [x] Big [docs](/docs) update
- [x] Make a test print

## September 2022

- [x] [Quest system](/docs/quest-trees)
- [x] [Docs page](/docs)
