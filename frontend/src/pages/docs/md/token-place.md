...
The default token.place origin is `https://token.place`, and the default API v1 chat model is
`llama-3.1-8b-instruct`. Deployment operators should override the origin and token.place model
at runtime with public, non-secret environment variables:
