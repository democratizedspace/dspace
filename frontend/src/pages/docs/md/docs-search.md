...
- **Feature filters**: Queries that include `has:` (for example, `has:image` or `has:link`) still
  filter by those features. The filter works alongside the text query, and snippets are hidden for
  these operator searches. The only supported predicates today are `has:image` and `has:link`.
