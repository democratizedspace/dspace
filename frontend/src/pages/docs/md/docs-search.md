...
- **Feature filters**: Queries that include `has:` (for example, `has:image`) still filter by those
  features. The two available predicates are `has:image` and `has:link`. The filter works alongside
  the text query, and snippets are hidden for these operator searches.
