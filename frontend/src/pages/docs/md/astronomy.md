...
- Dialogue `requiresItems` gates:
    ...
    - `pointer-route` → "Polaris is stable and the heading is repeatable." — planisphere star chart ×1, Polaris alignment note ×1
    - `tripod-route` → "Alignment note captured and Polaris confirmed." — Polaris alignment note ×1, basic telescope ×1
    - `log-evidence` → "Evidence logged and verified." — mission logbook ×1, mission log entry ×1, Polaris alignment note ×1
...
- Troubleshooting/safety branches:
    ...
    - Unsafe nights can only close out after mission-log evidence is produced.
- Grants:
    - Dialogue options/steps grantsItems: None
    - Quest-level `grantsItems`: None
- Rewards:
...
- Dialogue `requiresItems` gates:
    - `start` → "Give me both approaches." — planisphere star chart ×1, red flashlight ×1
    - `belt-route` → "Nebula structure is visible and steady." — planisphere star chart ×1, basic telescope ×1, constellation sketch set ×1
    - `wide-route` → "Glow confirmed after refocus." — basic telescope ×1, constellation sketch set ×1
    - `log-observation` → "Log entry ready for interpretation." — mission logbook ×1, mission log entry ×1, feather quill ×1
    - `interpret` → "Interpretation complete and reproducible." — mission log entry ×1, constellation sketch set ×1
...
- Dialogue `requiresItems` gates:
    ...
    - `chart-route` → "Ring plane resolved clearly." — planisphere star chart ×1, basic telescope ×1, constellation sketch set ×1
    - `focus-route` → "Focus lock holds and the rings are visible." — basic telescope ×1, constellation sketch set ×1
    - `log-observation` → "Log entry complete." — mission logbook ×1, mission log entry ×1, feather quill ×1
    - `interpret` → "Interpretation passes; session is reproducible." — mission log entry ×1, constellation sketch set ×1
