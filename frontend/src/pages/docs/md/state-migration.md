...
- **Failure behavior**: if persisting the migrated v3 snapshot fails, legacy
  `gameState`/`gameStateBackup` keys are left in place so users can retry migration
  instead of losing the source payload.
