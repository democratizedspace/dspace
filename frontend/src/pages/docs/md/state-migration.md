...
needs no manual action. When IndexedDB is unavailable (for example in some private
browsing modes), the game falls back to storing data in `localStorage` and displays a
warning because localStorage may not hold the full v3 game state.
