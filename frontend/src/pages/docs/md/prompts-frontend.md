...
DSPACE's UI is built with Svelte and Astro. Use this guide alongside
[Codex Prompts](/docs/prompts-codex) when working on files inside `frontend/`, including
Svelte components, pages, and styles. Changes should improve clarity, accessibility, or
performance while keeping tests green. To keep the prompt docs evolving, see the
[Codex meta prompt](/docs/prompts-codex-meta). If these templates drift, refresh them with
the [Codex Prompt Upgrader](/docs/prompts-codex-upgrader). For failing workflows, use the
[Codex CI-failure fix prompt](/docs/prompts-codex-ci-fix). For deeper a11y guidance see
