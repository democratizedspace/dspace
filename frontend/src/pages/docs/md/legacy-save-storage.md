...
- **v3 detection:** v3 checks cookies/localStorage for legacy artifacts; the migration scope for
  v1 is **items + currency balances**, and v3 parses both `item-<id>` and
...
The v1 numeric item IDs map 1:1 to v3 UUIDs in
[`frontend/src/utils/legacyV1ItemIdMap.ts`](https://github.com/democratizedspace/dspace/blob/v3/frontend/src/utils/legacyV1ItemIdMap.ts).
The migration uses this mapping to translate legacy `item-<id>` cookies into v3 inventory UUIDs.
...
- **/settings → Legacy save upgrades** shows a V1 card with the items listed above and the
  detected `dUSD` currency balance.
- **Currency balance:** v3 detection parses `currency-balance-dUSD`, and migration converts it into
  the v3 `dUSD` inventory item.
...
- `quest-` and `checkpoint-` cookies remain **out of scope** for migration by default.
- `process-` and `machine-lock-` localStorage keys are **not migrated**; they exist solely for
  ...
  Add quest/checkpoint cookies manually only when QA specifically needs to validate out-of-scope
  data handling.
...
3. **Check V1 detection UI:** in `/settings` → **Legacy save upgrades**, confirm:
    ...
    - `currency-balance-dUSD` is parsed and shown as a detected balance.
...
4. **Merge v1 into v3:** click **Merge v1 into current save**.
    ...
    - Wallet balances are migrated into the v3 `dUSD` inventory item.
...
copy/pastable. The seeder writes **only** `gameState` (not `gameStateBackup`). **Do not use real
...
```json
{
    ...
    "inventory": { "3": 120.5, "10": 2, "24": 100 },
    "quests": { "welcome/howtodoquests": { "finished": true, "itemsClaimed": ["welcome/howtodoquests-start-0"] } },
    "processes": {},
...
- **Banner + settings card:** v3 legacy detection inspects `gameState`/`gameStateBackup`, then
  checks `versionNumberString`/`versionNumber` **or** a v2-style shape (`inventory`/`quests`/
  `processes`) for a legacy match.  
...
```json
{
    "inventory": { "": 0, "3": 5, "10": 1, "85": 1 },
    "quests": {
        "hydroponics/basil": { "stepId": "grown" },
        "welcome/howtodoquests": { "finished": true, "itemsClaimed": ["welcome/howtodoquests-start-0"] }
    },
    "processes": {
        "processes/benchy": { "startedAt": 1672531200000, "duration": 3600000, "finished": false }
    },
    "openAI": { "apiKey": "REDACTED" },
    "experimentalFlags": { "legacyImport": true },
    "_meta": { "lastUpdated": 1672534800000 }
}
```
...
- V2 localStorage detection reads `gameState` / `gameStateBackup` and checks for a
  `versionNumberString`/`versionNumber` prefix **or** a v2-style shape with `inventory`, `quests`,
  and `processes`.
