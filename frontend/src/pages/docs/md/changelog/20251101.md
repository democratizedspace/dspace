...
-   `/docs/rockets` now ships with a guided model rocket upgrade plan covering
    fabrication, servo calibration, and field operations so players can iterate on
    real hardware progression without leaving hobbyist territory.
-   The rocketry questline now includes a "Guided Model Flight" mission that chains
    printing, electronics, pad checks, and a servo-stabilized launch into a single
