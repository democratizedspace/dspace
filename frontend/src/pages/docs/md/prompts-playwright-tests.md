...
>    `npm run test:ci`, and `npm run test:e2e:groups`.
...
Ensure `npm run lint`, `npm run type-check`, `npm run build`, `npm run test:ci`,
and `npm run test:e2e:groups` pass before committing.
