...
- Dialogue `requiresItems` gates:
    ...
    - `baseline` → "Captured baseline pair" — ground sensor calibration log ×1
    - `baseline` → "Baseline pair already logged" — ground sensor calibration log ×1
    - `interpret` → "Both readings are in tolerance" — ground sensor calibration log ×1
- Grants:
...
- Processes used:
    - [arduino-thermistor-read](/processes/arduino-thermistor-read)
        ...
        - Consumes: none
        - Creates: ground sensor calibration log ×1
