...
- Dialogue `requiresItems` gates:
    ...
    - `baseline` → "Captured baseline pair" — Arduino Uno ×1, thermistor calibration log ×1
    - `baseline` → "Baseline pair already logged" — Arduino Uno ×1, thermistor calibration log ×1
    - `interpret` → "Both readings are in tolerance" — Arduino Uno ×1, thermistor calibration log ×1
- Grants:
...
- Processes used:
    - [arduino-thermistor-read](/processes/arduino-thermistor-read)
        ...
        - Consumes: none
        - Creates: thermistor calibration log ×1
