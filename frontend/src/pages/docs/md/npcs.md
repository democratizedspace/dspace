...
process knowledge base through the default token.place provider or the optional OpenAI provider
selected in Settings. The chat payload now also includes your live quest progress,
...
current inventory snapshot. The default token.place Chat provider can use this context without an API key; if you select OpenAI in Settings, dChat uses your saved OpenAI key instead. dChat will be provided free of charge
