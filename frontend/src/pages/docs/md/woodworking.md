...
- Dialogue `requiresItems` gates:
    ...
    - `wobble-check` → "No wobble and edges are safe to touch" — tape measure ×1, safety goggles ×1, Step stool ×1
- Grants:
...
- Dialogue `requiresItems` gates:
    ...
    - `square-check` → "Square verified; glue-up cured safely" — Wood glue ×1, tape measure ×1, safety goggles ×1, Bookshelf ×1
- Grants:
...
- Dialogue `requiresItems` gates:
    ...
    - `level-check` → "Level checks passed and structure feels solid" — Handsaw ×1, Workbench ×1
- Grants:
...
- Grants:
    ...
    - Quest-level `grantsItems`: None
- Rewards:
    - Wood glue ×1, Sandpaper pack ×1, Picture frame ×1
- Processes used:
