<script>
    ...
    $: allProcessIds = new Set(allProcesses.map((process) => normalizeProcessId(process.id)));
    ...
    $: {
        if (mounted && openPreviewProcessId && !allProcessIds.has(openPreviewProcessId)) {
