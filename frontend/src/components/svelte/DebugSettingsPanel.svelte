<script>
    ...
    const CHAT_DEBUG_KEY = 'showChatPromptDebug';
...
<section class="panel" data-hydrated={hydrated ? 'true' : 'false'}>
    <div class="heading">
        <h2>Debug</h2>
        <p>Enable QA-friendly tools and debug surfaces across the UI.</p>
    </div>

    <label class="toggle">
        <div class="toggle__label">
            <span class="title">Show quest dependency map on /quests</span>
            <span class="subtitle">Adds the dependency graph overlay to the Quests page.</span>
        </div>
        <button
            type="button"
    ...
    <label class="toggle">
        <div class="toggle__label">
            ...
            <span class="subtitle">
                Highlight full chat prompt payloads with RAG vs main content.
            </span>
        </div>
        <button
            type="button"
            ...
            aria-label="Show debug prompt payloads on the chat page"
            disabled={loading}
            on:click={toggleChatDebug}
            data-testid="chat-prompt-debug-toggle"
        >
            <span aria-hidden="true" class="toggle__thumb"></span>
