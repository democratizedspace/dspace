<script>
    ...
    async function handleImageUpload(event) {
        ...
        if (file) {
            try {
                const result = await downsampleAndCompressToJpeg(file);
                ...
                validationErrors = {
                    ...validationErrors,
                    image: 'Image processing failed. Please try a different image.',
                };
            }
        } else {
            previewUrl = null;
    ...
    async function validateForm() {
        ...
        const usingAutogeneratedImage =
            !isEdit && !imageDataUrl && !previewUrl && payload.image === DEFAULT_QUEST_IMAGE;
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            const imageUrl = getImageForSave();
            const questData = {
                ...payload,
