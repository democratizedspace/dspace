<script>
    ...
    $: {
        const questIdString = questId != null ? String(questId) : null;
        availableQuests = questIdString
            ? allQuests.filter((quest) => String(quest.id) !== questIdString)
            : allQuests;
    }
    ...
    async function validateForm() {
        ...
        const questIdString = questId != null ? String(questId) : null;
        const dependencyQuests = questIdString
            ? questsForValidation.filter((quest) => String(quest.id) !== questIdString)
            : questsForValidation;
