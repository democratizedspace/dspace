...
lists missing assets referenced by quests or items.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Find quest and item entries that reuse or are missing image assets, including "
    ...
    find_parser = subparsers.add_parser(
        ...
        help="List duplicate and missing image URLs across quests and items",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        missing_files = find_missing_files(usages, args.root)

        if args.json:
            output = json.dumps(
                serialize_report(duplicates, identical_files, missing_files),
                indent=2,
            )
            print(output)
        else:
            output = format_duplicates(duplicates, identical_files, usages, missing_files)
            if output:
                print(output)
            else:
