...
when referenced by different paths under ``frontend/public``, and surfaces references whose image
files do not exist locally.
"""
...
def find_missing_images(usages: Mapping[str, List[ImageReference]], repo_root: Path) -> MissingImageMap:
    """Return image references whose files do not exist under ``frontend/public``."""

    missing: MissingImageMap = {}
...
def format_duplicates(
    ...
    missing_images: MissingImageMap | None = None,
) -> str:
    ...
    if missing_images:
        lines.append("")
        lines.append("Missing image files (referenced but not found on disk):")
    ...
    if missing_images:
        lines.append("")
        lines.append(f"Total missing image references: {missing_reference_count}")
