...
export function buildHarnessEnv(options, sourceEnv = process.env) {
...
function main() {
  ...
  const env = buildHarnessEnv(options);
  ...
  console.log(
    `[qa:remote-smoke] mutation=${options.mutate ? 'enabled (custom-item create/delete check included)' : 'disabled (non-mutating mode)'}`
  );
