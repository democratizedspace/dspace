...
function readRealV2Json(options) {
  ...
  if (!envValue) {
    if (!existsSync(DEFAULT_REAL_V2_FIXTURE)) {
      throw new Error(`Default real v2 fixture is missing: ${DEFAULT_REAL_V2_FIXTURE}`);
    }
    const fixturePayload = readFileSync(DEFAULT_REAL_V2_FIXTURE, 'utf8').trim();
    if (!fixturePayload) {
      throw new Error(`Default real v2 fixture is empty: ${DEFAULT_REAL_V2_FIXTURE}`);
    }
    try {
      JSON.parse(fixturePayload);
    } catch {
