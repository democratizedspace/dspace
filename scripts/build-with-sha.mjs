...
import { createKnownNodeWarningFilter } from '../frontend/scripts/utils/filter-known-node-warnings.mjs';
import {
...
const keepKnownNodeWarningLine = createKnownNodeWarningFilter();

const shouldKeepLine = (line) =>
  keepKnownNodeWarningLine(line) &&
...
try {
  await run(process.execPath, ['scripts/build-docs-rag-index.mjs']);
  await run(process.execPath, ['frontend/scripts/build-processes.mjs']);
  await run(
    process.execPath,
    [path.join('frontend', 'scripts', 'run-astro-build.mjs')],
    {
      filterOutput: true,
      cwd: process.cwd(),
    }
