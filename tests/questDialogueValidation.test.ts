...
const isTerminalNode = (node: DialogueNode) => {
  ...
  return (
    Boolean(node.terminal) || nodeType === 'terminal' || nodeType === 'exit'
  );
};
...
const requirementsForOption = (option: DialogueOption) => {
  if (option.type === 'process' && option.process) {
    ...
    for (const required of (process?.requireItems as ItemCount[] | undefined) ??
      ...
      requirements.set(
        required.id,
        (requirements.get(required.id) ?? 0) + required.count
      );
    }
    ...
    for (const consumed of (process?.consumeItems as ItemCount[] | undefined) ??
      ...
      requirements.set(
        consumed.id,
        (requirements.get(consumed.id) ?? 0) + consumed.count
      );
    }
...
const transitionGuarantees = (option: DialogueOption) => {
  if (option.type === 'process' && option.process) {
    ...
    for (const required of (process.requireItems as ItemCount[] | undefined) ??
      ...
      guarantees.set(
        required.id,
        (guarantees.get(required.id) ?? 0) + required.count
      );
    }
    ...
    for (const consumed of (process.consumeItems as ItemCount[] | undefined) ??
      ...
      guarantees.set(
        consumed.id,
        (guarantees.get(consumed.id) ?? 0) + consumed.count
      );
    }
    ...
    for (const created of (process.createItems as ItemCount[] | undefined) ??
      ...
      guarantees.set(
        created.id,
        (guarantees.get(created.id) ?? 0) + created.count
      );
    }
...
const getStateLockErrors = (
  ...
  const incomingTransitions = new Map<
    string,
    { from: string; guarantees: Map<string, number> }[]
  >();
  ...
  for (const [nodeId, node] of nodeMap.entries()) {
    ...
    for (const option of node.options ?? []) {
      ...
      transitions.push({
        from: nodeId,
        guarantees: transitionGuarantees(option),
      });
  ...
  for (const [targetNodeId, transitions] of incomingTransitions.entries()) {
    ...
    const hasEscapeOption = targetOptions.some(
...
export const validateQuestDialogueCompletable = (
  ...
  const nodeMap = new Map(
    normalizedNodes.map((node) => [node.id as string, node])
  );
  ...
  for (const nodeId of reachable) {
    ...
    const hasTransition = options.some(
      (option) =>
        option.type === 'finish' ||
        (option.goto ? nodeMap.has(option.goto) : false)
    );
  ...
  errors.push(
    ...getStateLockErrors(nodeMap, reachable, quest, questPath, startId)
  );
...
describe('quest dialogue completable validation', () => {
  it('flags reachable dialogue dead-ends and unreachable finish nodes', () => {
    ...
    expect(
      validateQuestDialogueCompletable(quest, 'fixtures/dead-end.json')
    ).toEqual([
      {
        ...
        reason: 'dead-end',
        snippet:
          'text="Probe is hot." options=[goto -> missing-node (Log temp)]',
      },
      {
        questId: 'fixture/dead-end',
        questPath: 'fixtures/dead-end.json',
  ...
  it('flags transitions that can enter an action-locked node', () => {
    ...
    expect(
      validateQuestDialogueCompletable(quest, 'fixtures/state-locked.json')
    ).toEqual([
      {
  ...
  it('treats process consumeItems as required for actionability checks', () => {
    ...
    expect(
      validateQuestDialogueCompletable(
        quest,
        'fixtures/state-locked-consume-items.json'
      )
    ).toEqual([
      {
  ...
  it('flags transitions from the start node that enter action-locked nodes', () => {
    const quest: QuestLike = {
      id: 'fixture/state-locked-from-start',
      start: 'start',
      dialogue: [
        {
          id: 'start',
          text: 'Entry',
          options: [{ type: 'goto', goto: 'craft', text: 'Continue' }],
        },
        {
          id: 'craft',
          text: 'Need a compost sifter to continue.',
          options: [
            {
              type: 'process',
    ...
    expect(
      validateQuestDialogueCompletable(
        quest,
        'fixtures/state-locked-from-start.json'
      )
    ).toEqual([
      {
        questId: 'fixture/state-locked-from-start',
        questPath: 'fixtures/state-locked-from-start.json',
        nodeId: 'craft',
        reason: 'state-locked',
        snippet:
          'entered from "start" but lacks guaranteed items for any option; text="Need a compost sifter to continue." options=[process -> finish (Sift compost)]',
      },
    ]);
  });
  ...
  it('keeps composting/turn-pile completable from temp node to finish', async () => {
    ...
    const quest = quests.find(
      (entry: QuestLike) => entry.id === 'composting/turn-pile'
    );
