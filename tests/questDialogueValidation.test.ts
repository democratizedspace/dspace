...
describe('quest dialogue completable validation', () => {
  ...
  it('requires a non-process fallback option on composting/turn-pile moisture node', async () => {
    const quests = await loadQuests();
    const quest = quests.find(
      (entry: QuestLike) => entry.id === 'composting/turn-pile'
    );
    ...
    const moistureNode = (quest?.dialogue ?? []).find(
      (node) => node.id === 'moisture'
    );
    expect(moistureNode).toBeDefined();

    const hasNonProcessFallback = (moistureNode?.options ?? []).some(
      (option) => option.type === 'goto' && option.goto === 'mix'
    );

    expect(hasNonProcessFallback).toBe(true);
  });
