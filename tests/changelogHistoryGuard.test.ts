...
const SNAPSHOT_PATH = path.join(
  __dirname,
  '../frontend/tests/fixtures/changelogSnapshots.json'
);
const ALLOWLIST_PATH = path.join(
  __dirname,
  '../frontend/tests/fixtures/changelogAllowlist.json'
);
const CORRECTIONS_PATH = path.join(
  __dirname,
  '../frontend/tests/fixtures/changelogCorrections.json'
);
const SNAPSHOT_HASH_PATH = path.join(
  __dirname,
  '../frontend/tests/fixtures/changelogSnapshotHashes.json'
);
const LATEST_FILE = '20260401';
...
describe('Historical changelog guard', () => {
  ...
  ) as Record<string, AllowlistEntry>;
  const corrections = JSON.parse(
    fs.readFileSync(CORRECTIONS_PATH, 'utf8')
  ) as Record<string, CorrectionEntry[]>;
  const snapshotHashes = JSON.parse(
    fs.readFileSync(SNAPSHOT_HASH_PATH, 'utf8')
  ) as Record<string, string>;
  ...
  it('validates corrections metadata format', () => {
    for (const entries of Object.values(corrections)) {
      ...
      for (const entry of entries) {
        expect(['spelling', 'whitespace', 'link', 'formatting']).toContain(
          entry.type
        );
