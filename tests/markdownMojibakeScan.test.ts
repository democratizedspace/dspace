...
const mojibakeTokens: MojibakeToken[] = [
    ...
    { token: '\u00e2\u009c\u0094', label: 'mojibake heavy check mark' },
...
describe('docs markdown mojibake scan', () => {
    it('rejects common mojibake markers in source content', () => {
        ...
        const markdownFiles = Array.from(new Set(filesByRoot.flatMap(({ files }) => files))).sort();
        ...
        expect(missingRoots, 'Docs roots should each contain at least one content file').toEqual(
            []
        );
