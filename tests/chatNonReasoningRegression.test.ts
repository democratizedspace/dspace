...
describe('chat non-reasoning regression probes', () => {
  ...
  it('replaces suspicious precision without citations with a safe fallback', async () => {
    ...
    const { GPT5Chat, safeFallbackMessage } = await import(
  ...
  it('answers completed quests using PlayerState when present', async () => {
    ...
    responsesCreateMock.mockImplementationOnce(async (payload) => {
      ...
      const stats = statsMatch ? JSON.parse(statsMatch[1].trim()) : null;
      const playerStateMatch = text.match(/PlayerState v[^\n]*:\n([\s\S]*)$/);
      const snapshot = playerStateMatch
        ? JSON.parse(playerStateMatch[1].trim())
        : null;
      return {
        output_text:
          `Completed quests: ${snapshot.questsFinished.join(', ')} ` +
          `(official completed=${stats?.completedOfficialQuestCount ?? 0})`,
      };
    });
