...
def test_cli_reports_duplicates(tmp_path: Path) -> None:
    ...
    assert "Duplicate image reference total: 2" in stdout
    assert "Total duplicates across sections: 2" in stdout
...
def test_cli_output_matches_logic_layer(tmp_path: Path) -> None:
    ...
    # Verify summary appears (3 uses - 1 = 2 duplicates)
    assert "Duplicate image reference total: 2" in text_output
    assert "Total duplicates across sections: 2" in text_output
