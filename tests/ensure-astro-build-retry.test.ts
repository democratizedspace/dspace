...
const createTempDir = () =>
  fs.mkdtempSync(path.join(os.tmpdir(), 'ensure-astro-build-retry-'));
...
describe('ensureAstroBuild retry behavior', () => {
  ...
  it('cleans dist/ once and retries once when first build failure contains ENOENT', () => {
    const retryableError = new Error(
      'Command failed: node scripts/run-frontend-build.mjs'
    ) as Error & { stderr?: Buffer };
    retryableError.stderr = Buffer.from(
      'ENOENT: no such file or directory, open dist/client/_astro/x'
    );
    const execSpy = vi
      .fn()
  ...
  it('does not retry when build failure is non-ENOENT', () => {
    ...
    nonRetryableError.stderr = Buffer.from(
