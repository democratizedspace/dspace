...
const buildQuestPrerequisiteClosure = (allQuests: Array<any>) => {
  const requiresByQuest = new Map(
    allQuests.map((quest) => [
      quest.id,
      new Set<string>(quest.requiresQuests ?? []),
    ])
  );
...
const detectSiblingTransformSoftLockConflicts = ({
  ...
  for (const process of allProcesses) {
    ...
    for (const createdItemId of toItemIds(process.createItems)) {
      ...
      if (!creatorsByItem.has(createdItemId))
        creatorsByItem.set(createdItemId, new Set<string>());
    ...
    for (const consumedItemId of toItemIds(process.consumeItems)) {
      for (const createdItemId of toItemIds(process.createItems)) {
        if (
          !consumedItemId ||
          !createdItemId ||
          consumedItemId === createdItemId
        )
          continue;

        const creatorProcessIds =
          creatorsByItem.get(createdItemId) ?? new Set<string>();
        if (creatorProcessIds.size !== 1 || !creatorProcessIds.has(processId))
          continue;
        ...
        for (const transformedQuest of transformedQuests) {
          ...
          for (const rawQuest of rawQuests) {
            ...
            const rawDeps =
              prerequisiteClosure.get(rawQuest.id) ?? new Set<string>();
            ...
            const rawDirectPrereqs =
              requiresByQuest.get(rawQuest.id) ?? new Set<string>();
...
describe('quest completion item availability', () => {
  ...
  it('requires post-process transformed items for quests gated behind prerequisite transformations', async () => {
    ...
    const displayQuest = quests.find(
      (quest: any) => quest.id === 'completionist/display'
    );
  ...
  it('keeps completionist/catalog gated on the polished award after the polish transform', async () => {
    const quests = await loadQuests();
    const catalogQuest = quests.find(
      (quest: any) => quest.id === 'completionist/catalog'
    );
    ...
    const rawAwardId = polishProcess.consumeItems?.find(
      (entry: any) => entry.id === 'c01676ec-27e5-4a53-9a47-24bf6c5a56a9'
    )?.id;
    const polishedAwardId = polishProcess.createItems?.find(
      (entry: any) => entry.id === '1030a6c5-88b9-46e9-b38a-b20d8d326764'
    )?.id;

    expect(rawAwardId).toBeDefined();
    expect(polishedAwardId).toBeDefined();

    const catalogRequiredItems = uniqueItemIds([
      ...getQuestLevelRequiredItemIds(catalogQuest),
      ...getRequiredItemsFromFinishReachableTransitions(catalogQuest),
      ...getFinishOptions(catalogQuest).flatMap((option: any) =>
        getRequiredItemIds(option)
      ),
    ]);

    expect(catalogRequiredItems).toContain(polishedAwardId);
    expect(catalogRequiredItems).not.toContain(rawAwardId);
  });
