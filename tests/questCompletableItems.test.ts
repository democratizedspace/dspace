...
const computeRenewableItems = ({
  ...
  const renewable = new Set<string>();

  for (const item of allItems) {
    if (item.price || item.priceExemptionReason === 'BETA_PLACEHOLDER') {
      renewable.add(item.id);
    }
  }
  ...
  while (changed) {
    ...
    for (const process of allProcesses) {
      ...
      if (!required.every((itemId) => renewable.has(itemId))) continue;
      for (const created of toItemIds(process.createItems)) {
        if (renewable.has(created)) continue;
        renewable.add(created);
        changed = true;
      }
    }
  }
...
const buildQuestPrerequisiteClosures = (quests: Array<any>) => {
  const byId = new Map(quests.map((quest) => [quest.id, quest]));
  const cache = new Map<string, Set<string>>();

  const getClosure = (questId: string, seen = new Set<string>()) => {
    if (cache.has(questId)) return cache.get(questId) as Set<string>;
    if (seen.has(questId)) return new Set<string>();

    seen.add(questId);
    const closure = new Set<string>();
    const quest = byId.get(questId);
    if (!quest) return closure;

    for (const dependency of quest.requiresQuests ?? []) {
      closure.add(dependency);
      const dependencyClosure = getClosure(dependency, seen);
      for (const nested of dependencyClosure) {
        closure.add(nested);
      }
    }

    cache.set(questId, closure);
    return closure;
  };
...
describe('quest completion item availability', () => {
  ...
  it('requires post-process transformed items for quests gated behind prerequisite transformations', async () => {
    const quests = await loadQuests();
    const reminderQuest = quests.find(
      (quest: any) => quest.id === 'completionist/reminder'
    );
    const polishQuest = quests.find(
      (quest: any) => quest.id === 'completionist/polish'
    );

    expect(reminderQuest).toBeDefined();
    expect(polishQuest).toBeDefined();
    expect(reminderQuest.requiresQuests ?? []).toContain(
    ...
    const reminderRequiredItems = uniqueItemIds([
      ...
      ...getFinishOptions(reminderQuest).flatMap((option: any) =>
        getRequiredItemIds(option)
      ),
    ]);
  ...
  it('prevents optional prerequisite chains from consuming non-renewable sibling quest gates', async () => {
    const quests = await loadQuests();
    const processMap = new Map(
      (processes as Array<any>).map((process) => [process.id, process])
    );
    const renewableItems = computeRenewableItems({
      allItems: items as Array<any>,
      allProcesses: processes as Array<any>,
    });
    const closures = buildQuestPrerequisiteClosures(quests);

    const requiredByQuest = new Map<string, Set<string>>();
    const consumedByQuest = new Map<string, Set<string>>();
    ...
    for (const quest of quests) {
      ...
      requiredByQuest.set(quest.id, new Set(requiredItems));

      const consumed = new Set<string>();
      for (const processId of getProcessIdsReferencedByQuest(quest)) {
        const process = processMap.get(processId);
        if (!process) continue;
        for (const itemId of toItemIds(process.consumeItems)) {
          consumed.add(itemId);
        }
      }
      consumedByQuest.set(quest.id, consumed);
    }

    const hazards: string[] = [];
    for (const target of quests) {
      const targetClosure = closures.get(target.id) ?? new Set<string>();
      const nonRenewableRequired = [
        ...(requiredByQuest.get(target.id) ?? new Set<string>()),
      ].filter((itemId) => !renewableItems.has(itemId));
      if (nonRenewableRequired.length === 0) continue;

      for (const candidate of quests) {
        if (candidate.id === target.id) continue;
        if (targetClosure.has(candidate.id)) continue;

        const candidateClosure =
          closures.get(candidate.id) ?? new Set<string>();
        const includesTargetPrereqs = [...targetClosure].every((id) =>
          candidateClosure.has(id)
        );
        if (!includesTargetPrereqs) continue;

        const extraChain = new Set<string>(candidateClosure);
        for (const questId of targetClosure) {
          extraChain.delete(questId);
        }
        extraChain.add(candidate.id);

        const consumedInExtraChain = new Set<string>();
        for (const questId of extraChain) {
          for (const itemId of consumedByQuest.get(questId) ??
            new Set<string>()) {
            consumedInExtraChain.add(itemId);
          }
        }

        for (const itemId of nonRenewableRequired) {
          if (!consumedInExtraChain.has(itemId)) continue;
          hazards.push(
            `Quest "${candidate.id}" can consume non-renewable item "${itemId}" required by sibling quest ` +
              `"${target.id}" after shared prerequisites ${[...targetClosure].join(', ') || '(none)'}.`
          );
        }
      }
    }
