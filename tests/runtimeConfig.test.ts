...
describe('runtime endpoints', () => {
  ...
  it('exposes production token.place defaults when runtime env is absent', async () => {
    ...
    expect(body.chat).toStrictEqual({ defaultProvider: 'token-place' });
  });

  it.each(['token-place', 'openai'] as const)(
    'resolves accepted Chat provider %s',
    (provider) => {
      expect(resolveRuntimeDefaultChatProvider(provider)).toBe(provider);
    }
  );

  it.each(['', ' openai', 'openai ', 'OPENAI', 'token.place', 'unknown'])(
    'rejects malformed explicit Chat provider %j',
    (provider) => {
      expect(() => resolveRuntimeDefaultChatProvider(provider)).toThrow(
        'DSPACE_DEFAULT_CHAT_PROVIDER must be exactly'
      );
    }
  );

  it('fails /config.json closed for an invalid explicit Chat provider', async () => {
    process.env.DSPACE_DEFAULT_CHAT_PROVIDER = 'invalid';
    const response = await getRuntimeConfig();
  ...
  it('exposes the configured OpenAI deployment default', async () => {
