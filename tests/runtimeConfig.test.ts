...
describe('runtime endpoints', () => {
  ...
  it.each([
    ...
    ['openai', 'openai'],
  ])('exposes resolved Chat default for %s', async (configured, expected) => {
    if (configured === undefined)
      delete process.env.DSPACE_DEFAULT_CHAT_PROVIDER;
    else process.env.DSPACE_DEFAULT_CHAT_PROVIDER = configured;
    const response = await getRuntimeConfig();
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      chat: { defaultProvider: expected },
    });
  });

  it.each([' openai', 'OPENAI', '', 'token-place\n'])(
    'fails closed for malformed explicit Chat default %j',
    async (configured) => {
      process.env.DSPACE_DEFAULT_CHAT_PROVIDER = configured;
      const response = await getRuntimeConfig();
