...
const hasItems = (
  ...
  requirements.every(
    (entry) => (inventory.get(entry.id) ?? 0) >= Number(entry.count ?? 0)
  );

const buyMissingRequirements = (
  inventory: Map<string, number>,
  requirements: ItemCount[] = []
) => {
  let changed = false;
  ...
  for (const [nodeId, node] of nodeById.entries()) {
    for (const option of node.options ?? []) {
      ...
      if (!reverseEdges.has(target))
        reverseEdges.set(target, new Set<string>());
...
const simulateQuestBody = (quest: QuestData, startingState: InventoryState) => {
  ...
  while (guard++ < 1000) {
    ...
    const processOptions = options.filter(
      (option) =>
        option.type === 'process' && typeof option.process === 'string'
    );
...
const computePrereqClosure = (questsById: Map<string, QuestData>) => {
  ...
  const visit = (questId: string): Set<string> => {
    ...
    if (visiting.has(questId))
      throw new Error(`Quest dependency cycle detected at ${questId}`);
...
const simulateInventoryBeforeQuest = ({
  ...
  const prerequisiteIds = [
    ...(prereqClosure.get(questId) ?? new Set<string>()),
...
describe('quest process necessity simulation', () => {
  ...
  it('requires at least one process run for quests that gate progression with process outputs', async () => {
    ...
    for (const quest of quests) {
      ...
      const gatingRequirements = getBlockingGotoRequirements(quest).filter(
        (item) => createdItemIds.has(item.id)
      );
